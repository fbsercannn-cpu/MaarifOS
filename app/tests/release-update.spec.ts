...
test("eski sürümden sonra tarihli notları bir kez gösterir ve ayarlarda korur", async ({
  ...
  await expect(updateDialog).toContainText("Sürüm 0.2.0");
  await expect(updateDialog).toContainText("28 Temmuz 2026");
  await expect(updateDialog).toContainText(
    "Çocuğu merkeze alan gözlem deneyimi",
  );
  await expect(updateDialog).toContainText("Sürüm 0.1.0 → 0.2.0");
  ...
  await expect(settings).toContainText("MaarifOS 0.2.0");
  await expect(settings).toContainText("28 Temmuz 2026");
  await settings.getByRole("button", {
    name: "Sürüm notlarını göster",
  }).click();
  await expect(settings).toContainText(
    "10 gözlem kategorisi ve 30 nötr öneri",
  );
});
...
test("sürüm kaydı olmayan mevcut Emine kurulumu ilk yükseltmeyi görür", async ({
  ...
  await expect(updateDialog).toBeVisible();
  await expect(updateDialog).toContainText("Sürüm 0.2.0");
  await expect(updateDialog).not.toContainText("Sürüm 0.1.0 → 0.2.0");
});
