...
import { synthesizeLocalResponse, type SynthesizedResponse } from "../components/pedagogical-ai-brain";
...
type AIGatewayResponse = { text: string; model: string };
...
export class SecureAIClient {
  ...
  static getActiveProvider(): SupportedAIProvider {
    purgeLegacyClientSecrets();
    return browserStorage()?.getItem(ACTIVE_PROVIDER_KEY) === "deepseek" ? "deepseek" : "local";
  }
    ...
    try {
      ...
      if (!response.ok || !value || typeof value.available !== "boolean") {
        return {
          available: false,
          provider: "none",
          reason: response.status === 401 ? "authentication_required" : "unavailable",
        };
      }
      return value;
    } catch {
  ...
  static async requestCompletion(
    ...
    const value = await response.json().catch(() => null) as (AIGatewayResponse & { error?: string }) | null;
