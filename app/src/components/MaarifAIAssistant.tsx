/**
 * MaarifAIAssistant.tsx — MaarifOS 0.74.0 APEX SÜRÜM
...
export function MaarifAIAssistant({
  ...
  const [providerId, setProviderId] = useState<ProviderId>(
    () => (localStorage.getItem(LS.PROVIDER) as ProviderId) || "local"
  );
  const [mode, setMode] = useState<"local" | "apikey" | "deeplink">(
    () => (localStorage.getItem(LS.MODE) as any) || "local"
  );
    ...
    return [
      {
        id: "welcome-1",
        role: "assistant",
        content: `👋 **Merhaba Değerli Öğretmenim!** Ben MaarifOS Pedagojik Yapay Zekâ Destek Asistanı.
MEB Talim ve Terbiye Kurulu 2026 Türkiye Yüzyılı Maarif Modeli çerçevesinde sınıfınızdaki en güçlü yardımcınızım.

✨ **Neler Yapabilirim?**
- **Sıfır İnternet / Sıfır API Key:** Kendi yerel zeka çekirdeğimle anında tam **EK-6 Günlük Plan**, **Anekdot**, **Karne Görüşü** ve **Veli Bülteni** üretirim.
- **Tek Tıkla Plana Aktar:** Ürettiğim planı tek dokunuşla doğrudan sisteminize enjekte edebilirsiniz.
- **Sesli Dikte (🎙️):** Sınıfta dolaşırken mikrofon düğmesine basıp konuşabilirsiniz!

Aşağıdaki hızlı reçetelerden birini seçebilir ya da sorunuzu yazabilirsiniz.`,
        timestamp: Date.now(),
        category: "general",
      },
    ];
    ...
    async (text: string) => {
      ...
      if (isExcelIntent) {
        try {
          ...
          exportParentContactExcel(students, classroom);
      ...
      // ─── %100 YEREL SENTETİK ZEKA MOTORU (VARSAYILAN & FAIL-SAFE) ───
      setTimeout(() => {
        ...
        setIsLoading(false);
      }, 400); // Doğal ve akıcı 400ms mikro-gecikme
    },
            ...
            <div className="maai-panel__header-left">
              ...
              <div className="maai-title-wrap">
                <div className="maai-panel__title">
                  MaarifOS AI
                  <span className="maai-version-chip">v0.75</span>
                </div>
                <div className="maai-panel__subtitle">
                  {provider.shortName} · {mode === "local" ? "Çevrimdışı Akıllı" : mode === "apikey" ? "Doğrudan API" : "Köprü"}
                </div>
              </div>
            </div>
                      ...
                      <div className="maai-msg__actions">
                        ...
                        {(msg.suggestedAction === "excel" || msg.content.includes("Excel Dosyası")) && (
                          <button
                            ...
                            onClick={() => {
                              exportParentContactExcel((window as any).__MAARIF_ACTIVE_STUDENTS__ || [], (window as any).__MAARIF_ACTIVE_CLASSROOM__);
