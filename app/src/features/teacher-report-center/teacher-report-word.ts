...
export function createTeacherReportWord(nodes: readonly SemanticPdfNode[], options: {landscape?: boolean; teacherName?: string} = {}): Uint8Array {
  ...
  const files = [
    ...
    { name: "word/document.xml", text: `<?xml version="1.0" encoding="UTF-8"?><w:document xmlns:w="${namespace}" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><w:body>${paragraphs}<w:sectPr><w:footerReference w:type="default" r:id="footer"/>${options.landscape ? '<w:pgSz w:w="16838" w:h="11906" w:orient="landscape"/>' : '<w:pgSz w:w="11906" w:h="16838"/>'}<w:pgMar w:top="640" w:right="640" w:bottom="800" w:left="640" w:header="320" w:footer="360"/></w:sectPr></w:body></w:document>` },
    { name: "word/styles.xml", text: wordDocumentStyles(20, true) },
    { name: "word/footer1.xml", text: wordRunningFooter(`${options.teacherName ? options.teacherName + " · Okul Öncesi Öğretmeni · " : ""}MaarifOS · Öğretmen dosyası`) },
  ];
