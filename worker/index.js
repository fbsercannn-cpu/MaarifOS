...
export const ACCOUNT_CANONICAL_PUBLIC_ORIGIN = "https://www.maarifos.com";
const ACCOUNT_PUBLIC_ORIGINS = new Set([
  "https://maarifos.com",
...
async function authorizeAIRequest(request, env, accountFetcher) {
  ...
  const nativeBearer = nativeOrigin !== null && NATIVE_APP_ORIGINS.has(nativeOrigin) &&
    authorization !== null && /^Bearer [A-Za-z0-9_-]{8,160}$/.test(authorization);
  const webCookie = cookie !== null && cookie.includes("__Host-maarifos-session=");
  if (!nativeBearer && !webCookie) return false;
  const statusUrl = new URL("/api/account/status", request.url);
  const statusRequest = new Request(statusUrl, {
    method: "GET",
    headers: {
      Accept: "application/json",
      ...(webCookie ? { Cookie: cookie, Origin: statusUrl.origin } : { Authorization: authorization, Origin: nativeOrigin }),
    },
  });
...
async function aiRateLimitKey(request) {
  const credential = request.headers.get("Cookie") ?? request.headers.get("Authorization") ?? "";
...
async function handleAIGatewayRequest(request, env, requestUrl, accountFetcher, aiFetcher) {
  ...
  const nativeOrigin = requestOrigin !== null && NATIVE_APP_ORIGINS.has(requestOrigin);
  const originRejected = request.method === "POST"
    ? requestOrigin !== requestUrl.origin && !nativeOrigin
    : requestOrigin !== null && requestOrigin !== requestUrl.origin && !nativeOrigin;
  if (originRejected) {
  ...
  try {
    ...
    if (!upstream.ok) {
      // Status and selected public model are safe operational diagnostics. Never
      // log the prompt, provider body, authorization header or account cookie.
      console.error(`DeepSeek upstream rejected ${configuration.model} with HTTP ${upstream.status}.`);
      return aiGatewayError(502, "Yapay zeka sağlayıcısı yanıt veremedi.", `provider_http_${upstream.status}`);
    }
...
async function proxyAccountRequest(request, env, requestUrl, routingPath, fetcher) {
  ...
  // Google OAuth is registered against the canonical www.com origin. Requests
  // may enter through any published MaarifOS domain, so normalize only those
  // host-bound origins before forwarding. Unknown/test origins stay untouched.
  if (
    ACCOUNT_PUBLIC_ORIGINS.has(requestUrl.origin) &&
    ACCOUNT_PUBLIC_ORIGINS.has(headers.get("Origin"))
  ) {
    headers.set("Origin", ACCOUNT_CANONICAL_PUBLIC_ORIGIN);
  }
  const callerOrigin = request.headers.get("Origin");
  const nativeCaller = callerOrigin !== null && NATIVE_APP_ORIGINS.has(callerOrigin);
  const authorization = request.headers.get("Authorization");
  if (nativeCaller && authorization !== null && /^Bearer [A-Za-z0-9_-]{8,160}$/.test(authorization)) {
  ...
  try {
    ...
    if (nativeCaller) {
      responseHeaders.set("Access-Control-Allow-Origin", callerOrigin);
  ...
  return {
    async fetch(request, env) {
      ...
      if (routingPath !== null && AI_GATEWAY_PATHS.includes(routingPath)) {
        const callerOrigin = request.headers.get("Origin");
        if (request.method === "OPTIONS" && callerOrigin !== null && NATIVE_APP_ORIGINS.has(callerOrigin)) {
        ...
        const response = await handleAIGatewayRequest(request, env, requestUrl, accountFetcher, aiFetcher);
        if (callerOrigin !== null && NATIVE_APP_ORIGINS.has(callerOrigin)) {
          const headers = new Headers(response.headers);
      ...
      if (routingPath !== null && isAccountNetworkPath(routingPath)) {
        ...
        if (request.method === "OPTIONS" && callerOrigin !== null && NATIVE_APP_ORIGINS.has(callerOrigin)) {
          ...
          if (requestedMethod !== "POST" && requestedMethod !== "GET") {
            return accountProxyError(405, "Native hesap isteğinin yöntemi desteklenmiyor.", "method_not_allowed");
          }
          if (requestedHeaders.some((name) => !["authorization", "content-type", "accept"].includes(name))) {
            return accountProxyError(403, "Native hesap isteği başlıkları doğrulanamadı.", "invalid_headers");
          }
          return withSecurityHeaders(new Response(null, { status: 204, headers: {
            "Access-Control-Allow-Origin": callerOrigin,
            "Access-Control-Allow-Methods": "GET, POST",
