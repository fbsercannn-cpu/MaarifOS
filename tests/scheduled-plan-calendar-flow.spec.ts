...
test("mobil Planlar → okul etkinliği → tarih → akış → düzenle → reload zinciri kimlik ve provenance korur", async ({
  ...
  await calendar.getByRole("gridcell", { name: /^08\/09\/2026/ }).click();
  ...
  await expect(planSheet).toContainText("08/09/2026");
  ...
  await reloadedCalendar.getByRole("gridcell", { name: /^08\/09\/2026/ }).click();
  await expect(reloadedCalendar.getByTestId("calendar-scheduled-plan")).toHaveCount(0);
  await reloadedCalendar.getByRole("gridcell", { name: /^09\/09\/2026/ }).click();
