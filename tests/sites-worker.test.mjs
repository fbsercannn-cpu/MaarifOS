...
import worker, {
  ...
  ACCOUNT_CANONICAL_PUBLIC_ORIGIN,
...
test("AI gateway accepts only server bindings and approved models", () => {
  assert.deepEqual(AI_GATEWAY_MODELS, ["deepseek-flash", "deepseek-v4-pro"]);
...
test("AI gateway keeps provider secret server-side and returns bounded text", async () => {
  ...
  const browserStyleStatus = await secured.fetch(new Request("https://example.test/api/ai/status", {
    headers: { Cookie: "__Host-maarifos-session=synthetic-session" },
  }), env);
  assert.equal(browserStyleStatus.status, 200);
  assert.deepEqual(await browserStyleStatus.json(), { available: true, provider: "deepseek", model: AI_GATEWAY_MODELS[0] });
  const response = await secured.fetch(aiRequest("/api/ai/chat", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ prompt: "Kurgu eğitim taslağı" }),
...
test("native AI uses the secure account bearer and exact-origin CORS", async () => {
  let accountCalls = 0;
  const secured = createSitesWorker({
    accountFetcher: async (request) => {
      accountCalls++;
      assert.equal(request.headers.get("authorization"), "Bearer native_ai_fixture");
      assert.equal(request.headers.get("origin"), "https://localhost");
      return connectedAccountFetcher();
    },
  });
  const preflight = await secured.fetch(new Request("https://www.maarifos.com/api/ai/status", {
    method: "OPTIONS",
  ...
  const response = await secured.fetch(new Request("https://www.maarifos.com/api/ai/status", {
    headers: { Origin: "https://localhost", Authorization: "Bearer native_ai_fixture" },
  }), aiGatewayEnvironment());
  assert.equal(response.status, 200);
  assert.equal(accountCalls, 1);
  assert.equal(response.headers.get("access-control-allow-origin"), "https://localhost");
  assert.deepEqual(await response.json(), { available: true, provider: "deepseek", model: AI_GATEWAY_MODELS[0] });
});

test("AI gateway exposes only a safe provider status code when DeepSeek rejects billing", async () => {
  const secured = createSitesWorker({
    accountFetcher: connectedAccountFetcher,
    aiFetcher: async () => Response.json({ error: { message: "private upstream detail" } }, { status: 402 }),
  });
  const response = await secured.fetch(aiRequest("/api/ai/chat", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  assert.equal(value.code, "provider_http_402");
  assert.equal(JSON.stringify(value).includes("private upstream detail"), false);
});
...
test("published MaarifOS domains use the registered canonical Google OAuth origin", async () => {
  for (const origin of [
    "https://maarifos.com",
    "https://www.maarifos.com",
    "https://maarifos.net",
    "https://www.maarifos.net",
  ]) {
    let forwardedOrigin = null;
    const accountWorker = createSitesWorker({ accountFetcher: async (request) => {
      forwardedOrigin = request.headers.get("origin");
      return Response.json({ authorizationUrl: "https://accounts.google.com/o/oauth2/v2/auth?state=synthetic" });
    } });
    const response = await accountWorker.fetch(new Request(`${origin}/auth/google/start`, {
      method: "POST",
      body: JSON.stringify({ purpose: "login" }),
      headers: { Origin: origin, "Content-Type": "application/json" },
    }), accountProxyEnvironment());
    assert.equal(response.status, 200, origin);
    assert.equal(forwardedOrigin, ACCOUNT_CANONICAL_PUBLIC_ORIGIN, origin);
  }
});

test("native account CORS is exact-origin, bounded and forwards only valid bearer credentials", async () => {
  const env = accountProxyEnvironment();
  const preflightWorker = createSitesWorker();
  const preflight = await preflightWorker.fetch(new Request("https://www.maarifos.com/api/account/native/exchange", {
    method: "OPTIONS",
    headers: {
  ...
  const accountWorker = createSitesWorker({ accountFetcher: async (request) => {
    calls++;
    assert.equal(request.headers.get("origin"), "https://localhost");
    assert.equal(request.headers.get("authorization"), "Bearer native_fixture_token");
    return Response.json({ available: true, status: "connected", user: { subject: "fixture", displayName: "Öğretmen" }, driveConnected: false });
  } });
  const response = await accountWorker.fetch(new Request("https://www.maarifos.com/api/account/status", {
    headers: { Origin: "https://localhost", Authorization: "Bearer native_fixture_token" },
  }), env);
  assert.equal(calls, 1);
  assert.equal(response.headers.get("access-control-allow-origin"), "https://localhost");
  assert.equal(response.headers.get("access-control-allow-credentials"), null);

  const rejected = await preflightWorker.fetch(new Request("https://www.maarifos.com/api/account/status", {
    method: "OPTIONS",
