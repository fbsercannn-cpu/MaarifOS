...
import { observationOutcomeCandidate, saveObservationOutcomePackage, OBSERVATION_FAMILY_BULLETIN_REPORT_TYPE } from "../../src/features/action-center/observation-outcome-package.ts";
...
function withConfirmedProgramLink(store) {
  const observation = store.snapshot.observations[0];
  observation.studentIds = [studentId];
  observation.observedAt = "2026-09-10T08:00:00.000Z";
  store.snapshot.evidenceCurriculumLinks.push({
    ...base, ...scope, id: uid(9010), observationId, approvedByUserId: uid(9011),
    framework: "meb_2024", programLabel: "MEB 2024", catalogId: "kurgu-katalog",
    sourceVersion: "2024.1", referenceCode: "EÇE.K1", referenceTitle: "Kendini sözlü olarak ifade eder",
    confirmedAt: "2026-09-10T08:30:00.000Z", confirmationMethod: "teacher-confirmed",
...
test("Gözlem tek onayla kaynaklı değerlendirme ve veli bültenini atomik kaydeder; tekrar tıklama mükerrer üretmez", async () => {
  const store = withConfirmedProgramLink(fixture());
  const candidate = observationOutcomeCandidate(store.snapshot, studentId, observationId);
  assert.ok(candidate);
  assert.equal(candidate.generationMode, "local-safe-fallback");
  assert.match(candidate.sourceDisclosure, /ham gözlem/u);
  assert.match(candidate.familyBulletinText, /şu sınıf gözlemini paylaşmak isteriz/u);
  assert.doesNotMatch(candidate.familyBulletinText, /olgunluk sergiledi/u);
  const results = await Promise.all([
    saveObservationOutcomePackage(store, { ...candidate, now }),
    saveObservationOutcomePackage(store, { ...candidate, now }),
  ]);
  assert.deepEqual(results.map(result => result.alreadyCompleted), [false, true]);
  const packageDrafts = store.snapshot.reportDrafts.filter(record => record.reportType === "evidence-assessment" || record.reportType === OBSERVATION_FAMILY_BULLETIN_REPORT_TYPE);
  assert.equal(packageDrafts.length, 2);
  assert.equal(packageDrafts.filter(record => record.reportType === "evidence-assessment").length, 1);
  assert.equal(packageDrafts.filter(record => record.reportType === OBSERVATION_FAMILY_BULLETIN_REPORT_TYPE).length, 1);
  assert.ok(packageDrafts.every(record => (record.observationIds ?? record.selectedObservationIds).includes(observationId)));
  assert.equal(store.snapshot.observations[0].rawText, "  Arkadaşına resmi anlattı.\nKendi sözünü kullandı.  ");
});
test("öğretmenin düzenlediği DeepSeek taslakları model ve gizlilik kaynağıyla birlikte kaydolur", async () => {
  const store = withConfirmedProgramLink(fixture());
  const local = observationOutcomeCandidate(store.snapshot, studentId, observationId);
  assert.ok(local);
  const input = {
    ...local,
    assessmentText: "Öğretmenin düzenlediği somut değerlendirme metni, gözlenen anlatımı ve izlenecek sonraki fırsatı açıklar.",
    familyBulletinText: "Öğretmenin düzenlediği veli bülteni, sınıftaki somut gözlemi ve evde isteğe bağlı anlatım fırsatını paylaşır.",
    generationMode: "deepseek",
    aiModel: "deepseek-flash",
    privacyDisclosure: "İsteme çocuk adı veya iletişim bilgisi eklenmedi.",
  };
  await saveObservationOutcomePackage(store, { ...input, now });
  const assessment = store.snapshot.reportDrafts.find(record => record.reportType === "evidence-assessment");
  const bulletin = store.snapshot.reportDrafts.find(record => record.reportType === OBSERVATION_FAMILY_BULLETIN_REPORT_TYPE);
  assert.equal(assessment.teacherAssessmentText, input.assessmentText);
  assert.equal(bulletin.editableSections.familyBulletinText, input.familyBulletinText);
  assert.equal(bulletin.editableSections.aiGenerationMode, "deepseek");
  assert.equal(bulletin.editableSections.aiModel, "deepseek-flash");
  assert.equal(bulletin.editableSections.privacyDisclosure, input.privacyDisclosure);
});
test("Değerlendirme ve bülten paketinde ikinci yazma başarısızsa hiçbir parça kalmaz", async () => {
  const store = withConfirmedProgramLink(fixture());
  const candidate = observationOutcomeCandidate(store.snapshot, studentId, observationId);
  assert.ok(candidate);
  store.failCollection = "reportDrafts";
  await assert.rejects(() => saveObservationOutcomePackage(store, { ...candidate, now }), /Kurgu yazma hatası/u);
  assert.equal(store.snapshot.reportDrafts.length, 0);
  assert.equal(store.snapshot.settings.filter(record => record.settingType === "local-teacher-identity-v1").length, 0);
});
