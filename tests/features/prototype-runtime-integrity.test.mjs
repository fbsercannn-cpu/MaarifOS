...
test("Gözlem gözlem seçimi chooser'ı evidence geçiş kilidiyle kapatır ve iki dialogu birlikte bırakmaz", () => {
  ...
  assert.match(
    observationOpen,
    /const liveEvidence =\s*evidenceWorkspace\.civilDate === today\s*\? evidenceWorkspace\s*:\s*await loadEvidenceWorkspace\(store, \{ now \}\)/,
  );
  assert.match(observationOpen, /resolveObservationContext\(liveEvidence/);
  assert.match(
    observationOpen,
    /surfaceTransitionRef\.current = "evidence-flow";\s*setCaptureMenuOpen\(false\);[\s\S]{0,120}setDataBusy\(true\)/,
  );
