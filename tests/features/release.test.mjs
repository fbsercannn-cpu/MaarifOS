...
test("güncel sürüm kullanıcıya gösterilecek eksiksiz Türkçe metadata taşır", () => {
  assert.equal(CURRENT_RELEASE.version, "0.75.3");
  assert.equal(CURRENT_RELEASE.releasedOn, "2026-09-25");
  assert.equal(
    CURRENT_RELEASE.title,
    "Hızlı Gözlem ve kurumsal bekleme deneyimi",
  );
...
test("ilk kurulumu onaylar ve sonraki açılışı güncel sürüm olarak tanır", () => {
  ...
  assert.deepEqual(
    ...
    {
      schemaVersion: 1,
      firstSeenVersion: "0.75.3",
      acknowledgedVersion: "0.75.3",
  ...
  assert.equal(state.shouldPresent, false);
  assert.equal(state.previousVersion, "0.75.3");
});
...
test("önceki sürümü gerçek güncelleme olarak ayırır ve sürüm notlarını sunar", () => {
  ...
  assert.equal(state.release.version, "0.75.3");
});
