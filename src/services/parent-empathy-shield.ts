...
export function generateParentEmpathyDigest(input: ParentEmpathyDigestInput): ParentEmpathyDigest {
  ...
  const objectiveObservation = sanitized.replace(/[.!?]+$/u, "");
  // Cümle 1 yalnız kaynak gözlemi aktarır; gözlemden çıkarılamayan kişilik veya
  // gelişim hükmü eklemez.
  const sentence1 = `Değerli Velimiz, bugün ${firstName} için şu sınıf gözlemini paylaşmak isteriz: ${objectiveObservation}.`;
  // Cümle 2, aileye zorunluluk yüklemeden kısa bir ev seçeneği sunar.
  const sentence2 = `İsterseniz evde ${homeActivity} yaparak bu deneyimi (${effectiveValue}) destekleyebilirsiniz. Sevgilerimizle, Maarif Sınıfı.`;
