...
export class StudentSensitiveVault {
  ...
  /** Aynı canlı kasada doğrulanmış tam zarf için in-memory rollback tanığı. */
  private readonly acceptedEnvelopeFingerprints = new Map<string, string>();
  ...
  async prepareCollectionRetirements(collection: string, records: readonly StoredRecord[]): Promise<LocalVaultRetirementPreparation> {
    const context = await this.getExistingV2Context();
    const receipt = await prepareLocalVaultRecordGenerationRetirements(await this.openKeyDatabase(), {
    ...
    for (const key of this.acceptedEnvelopeFingerprints.keys()) {
      if (key.startsWith(prefix) && !presentIds.has(key.slice(prefix.length))) {
        this.acceptedEnvelopeFingerprints.delete(key);
      }
    }
    return receipt;
  }
  ...
  /**
   * Append-only commitlerde mevcut kayıt nesilleri değişmediği ve hiçbir kimlik
   * kaldırılmadığı için koleksiyon çapında emeklilik uzlaşması gerekmez. Yeni
   * zarflar yine rollback ledger'a kabul edilir; güvenlik sınırı gevşetilmez.
   */
  async acceptAppendedCollectionRecords(
    collection: string,
    records: readonly StoredRecord[],
  ): Promise<void> {
    const context = await this.getExistingV2Context();
    for (const record of records) {
      await this.acceptV2Envelope(record, collection, record.id, context);
    }
  }
  ...
  private async sealV2Record(
    ...
    // Zarf özeti, ana veri commit'inden sonra nesli kabul eden tek key-DB
    // transaction'ında rezervasyona bağlanır. Böylece WebKit'te her kayıt için
    // ayrı bir strict-durability bind turu oluşmaz; crash halinde null özetli
    // rezervasyon fail-closed kalır ve yalnız doğrulanmış zarfla tamamlanır.
    return sealed;
  }
  ...
  private async acceptV2Envelope(
    ...
    const envelopeSha256 = await this.sha256(record);
    const cacheKey = `${collection}\u0000${expectedRecordId}`;
    const fingerprint = `${envelope.recordGeneration ?? 0}\u0000${envelopeSha256}`;
    if (this.acceptedEnvelopeFingerprints.get(cacheKey) === fingerprint) return;
    ...
    this.acceptedEnvelopeFingerprints.set(cacheKey, fingerprint);
  }
  ...
  close(): void {
    ...
    this.acceptedEnvelopeFingerprints.clear();
    if (current) {
      void current
