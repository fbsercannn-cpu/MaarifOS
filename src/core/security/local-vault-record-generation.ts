...
export async function acceptLocalVaultRecordGeneration(
  ...
  try {
    ...
    const reservation = await requestResult(
      reservationStore.get(reservationId(recordContextId, generation)) as IDBRequest<
    ...
    if (
      ...
      (reservation.envelopeSha256 !== null &&
    ...
    if (reservation.envelopeSha256 === null) {
      await requestResult(
