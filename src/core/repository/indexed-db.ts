    ...
    ClassroomDataQueryRepository
{
    ...
    const database = await this.open();
    {
      const execute = async (): Promise<T> => {
        ...
        if (mode === "readwrite") {
          ...
          const changedCollections = scopedCollections.filter(
            (collection) =>
              !recordsEqual(
          ...
          if (changedCollections.length === 0) return result;
          const changedRawState = await this.sealSensitiveCollections(
            finalPublicState,
            changedCollections,
            publicState,
            baseline,
          );
          const finalRawState: RawCollectionState = {
  ...
  private async commitRawCollections(
    ...
    changedCollections: ReadonlySet<CollectionName> = new Set(collections),
  ): Promise<void> {
    const scopedCollections = uniqueCollections(collections);
    ...
    const appendOnlyRecords = new Map<CollectionName, StoredRecord[]>();
    ...
    try {
      for (const collection of scopedCollections) {
        if (!changedCollections.has(collection)) continue;
        ...
        const appendOnly =
          ...
          before.every((record) => {
            const matchingAfter = afterById.get(record.id);
            return matchingAfter !== undefined && recordsEqual([record], [matchingAfter]);
          });
        if (appendOnly) {
          appendOnlyRecords.set(
            collection,
            after.filter((record) => !beforeById.has(record.id)),
          );
          continue;
        }
        preparations.push(
          await this.getSensitiveVault().prepareCollectionRetirements(
            collection,
            after,
          ),
        );
      ...
      for (const collection of scopedCollections) {
        if (!changedCollections.has(collection)) continue;
        const appended = appendOnlyRecords.get(collection);
        if (appended) {
          await this.getSensitiveVault().acceptAppendedCollectionRecords(
            collection,
            appended,
          );
        } else {
          await this.getSensitiveVault().acceptCollectionRecords(
            collection,
            finalState[collection] ?? [],
          );
        }
      }
      if (recovery) {
