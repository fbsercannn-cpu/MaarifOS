...
import { formatTurkishDate } from "../../shared/turkish-date.ts";
...
export interface TodayStudentCard {
  ...
  contacts?: DashboardStudent["contacts"];
...
function periodLabel(periodStart: string, periodEnd: string): string {
  return `${formatTurkishDate(periodStart)}–${formatTurkishDate(periodEnd)}`;
}

function shortCivilDateLabel(civilDate: string): string {
  return formatTurkishDate(civilDate);
}
...
export function createTodayStudentCards(
  ...
  return students.map((student) => ({
    ...
    ...(student.optionalCode ? { optionalCode: student.optionalCode } : {}),
