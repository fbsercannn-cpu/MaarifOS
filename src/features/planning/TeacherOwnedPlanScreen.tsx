...
export function TeacherOwnedPlanScreen({
  ...
  const locallyCreatedAnnualPlanIdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (
      annualPlanId &&
      locallyCreatedAnnualPlanIdRef.current === annualPlanId
    ) {
      // Zincir bu açık ekranda zaten doğrulanıp state'e alındı. Üst çalışma
      // alanının gecikmeli kimlik güncellemesi aynı şifreli snapshot'ı yeniden
      // okuyup belge üretimiyle yarışmamalı.
      locallyCreatedAnnualPlanIdRef.current = null;
      return;
    }
    let active = true;
    setBusy(true);
  ...
  const createStarterPlan = async () => {
    ...
    try {
      // İlk dokunuşta yalnız öğretmenin ekranda gördüğü başlangıç aylarını
      // atomik olarak kaydet. Eylül–Haziran omurgasının kalan ayları hemen
      // alttaki açık tamamla eylemiyle eklenir. Böylece düşük güçlü WebKit
      // cihazlarında onlarca haftayı tek tıklamada şifreleyip yazmayız.
      const starterMonthKeys = new Set(
        [starter.monthKey, starter.nextMonthKey].filter(
          (monthKey): monthKey is string => typeof monthKey === "string",
        ),
      );
      const authoredYearDrafts = fullYearMonthDrafts
        .filter((month) => starterMonthKeys.has(month.monthKey))
        .map((month) => ({
        ...month,
        teacherContent:
      ...
      locallyCreatedAnnualPlanIdRef.current = graph.annual.id;
      setTeacherGraph(graph);
