...
import { DeepSeekTeacherWorkCenter } from "../ai-work-center/DeepSeekTeacherWorkCenter.tsx";
import { DeepSeekPlanReviewDialog } from "../ai-work-center/DeepSeekPlanReviewDialog.tsx";
import {
  createDeepSeekTeacherWork,
  type DeepSeekTeacherWork,
} from "../ai-work-center/deepseek-teacher-work.ts";
...
export interface SimpleTodayScreenActions {
  ...
  onPlanRevisionSaved(): void | Promise<void>;
}
...
export function SimpleTodayScreen({
  ...
  const [pendingAiPlan, setPendingAiPlan] = useState<DailyPlanRecord | null>(null);
  const [planReviewPlanId, setPlanReviewPlanId] = useState<string | null>(null);

  const deepSeekTasks = useMemo(
    () => createDeepSeekTeacherWork({
      civilDate: todayCivilDate,
      ageBand: classroom?.ageGroup,
      programLabel: compactTodayProgramLabel(classroom?.curriculumProgram),
      studentCount: model.students.length,
      attendance: {
        marked: model.attendance.marked,
        total: model.attendance.total,
      },
      pendingObservationCount: model.pendingObservationCount,
      hasDailyPlan: model.teacherCycle.daily.planId !== null,
    }),
    [
      classroom?.ageGroup,
      classroom?.curriculumProgram,
      model.attendance.marked,
      model.attendance.total,
      model.pendingObservationCount,
      model.students.length,
      todayCivilDate,
      model.teacherCycle.daily.planId,
    ],
  );

  const handleTodayAiGenerate = async (customPrompt?: string) => {
    const promptToUse = (customPrompt || todayAiPrompt).trim() || "Günün mevsimine ve TYMM okul öncesi programına uygun bütünleştirilmiş günlük plan";
    setTodayAiBusy(true);
    setTodayAiNotice("⚡ DeepSeek güvenli ağ geçidi deneniyor; gerekirse cihaz içi motor devreye girecek…");
    try {
      ...
      setPendingAiPlan(newPlan);
      setTodayAiNotice("Taslak hazır. İnceleyip açıkça kaydetmeden bugünün planı değişmez.");
    } catch (err: any) {
  ...
  const savePendingAiPlan = () => {
    if (!pendingAiPlan) return;
    upsertDailyPlan(pendingAiPlan);
    setDailyPlans(loadDailyPlans());
    setPendingAiPlan(null);
    setTodayAiPrompt("");
    setTodayAiNotice("Plan taslağı öğretmen onayıyla bugüne kaydedildi.");
    void actions.onPlanRevisionSaved();
  };

  const handleDeepSeekTask = async (task: DeepSeekTeacherWork) => {
    if (task.id === "review-daily-plan") {
      if (!preferenceStore || !model.teacherCycle.daily.planId) {
        setTodayAiNotice("Plan incelemesi için gerçek öğretmen günlük planı bulunamadı.");
        return;
      }
      setTodayAiNotice("");
      setPlanReviewPlanId(model.teacherCycle.daily.planId);
      return;
    }
    if (task.kind === "plan") {
      await handleTodayAiGenerate("Bugünün sınıf bağlamına uygun oyun temelli ve bütünleştirilmiş günlük plan");
      return;
    }
    if (task.kind === "observation") {
      window.dispatchEvent(new CustomEvent("maarif_open_ai_anecdote"));
      return;
    }
    window.dispatchEvent(new CustomEvent("maarif_open_ai_assistant", {
      detail: { query: task.query, source: "teacher-work-center" },
    }));
  };
  ...
  return (
    ...
    >
      <header className="simple-today__header">
        ...
        <div className="simple-today__header-actions">
          <button
            type="button"
            ...
            className="simple-today__assistant-button"
          >
      ...
      {classroom ? (
        <DeepSeekTeacherWorkCenter
          tasks={deepSeekTasks}
          busy={todayAiBusy}
          notice={todayAiNotice}
          onSelect={handleDeepSeekTask}
        />
      ) : null}

      {planReviewPlanId && preferenceStore ? (
        <DeepSeekPlanReviewDialog
          store={preferenceStore}
          planId={planReviewPlanId}
          onClose={() => setPlanReviewPlanId(null)}
          onSaved={({ revisionNumber }) => {
            setTodayAiNotice(`Planın ${revisionNumber}. revizyonu öğretmen onayıyla kaydedildi.`);
            void actions.onPlanRevisionSaved();
          }}
        />
      ) : null}
      ...
      {pendingAiPlan ? (
        ...
        >
          <section
            role="dialog"
            aria-modal="true"
            aria-labelledby="ai-plan-draft-title"
            aria-describedby="ai-plan-draft-description"
            data-testid="ai-plan-draft-dialog"
          ...
          >
            <header style={{ padding: "20px 20px 14px", borderBottom: "1px solid #e2e8f0" }}>
              <span style={{ color: "#047857", fontSize: "0.76rem", fontWeight: 800, letterSpacing: "0.04em" }}>
                AI TASLAĞI · ÖĞRETMEN ONAYI GEREKİR
              </span>
              <h2 id="ai-plan-draft-title" style={{ margin: "6px 0 4px", fontSize: "1.24rem" }}>
                {pendingAiPlan.topic}
              </h2>
              <p id="ai-plan-draft-description" style={{ margin: 0, color: "#64748b", lineHeight: 1.5 }}>
                Bu içerik MaarifOS tarafından taslak olarak hazırlanmıştır; henüz bugünün planına kaydedilmemiştir.
              </p>
            </header>

            <div style={{ padding: "16px 20px", display: "grid", gap: "12px" }}>
              <div style={{ padding: "12px", borderRadius: "12px", background: "#f0fdfa" }}>
                <strong>Araştırma sorusu</strong>
                <p style={{ margin: "5px 0 0", lineHeight: 1.55 }}>{pendingAiPlan.researchQuestion}</p>
              </div>
              <div style={{ padding: "12px", borderRadius: "12px", background: "#f8fafc" }}>
                <strong>Etkinlik</strong>
                <p style={{ margin: "5px 0 0", lineHeight: 1.55 }}>{pendingAiPlan.activityName}</p>
              </div>
              <div style={{ padding: "12px", borderRadius: "12px", background: "#f8fafc" }}>
                <strong>Uygulama özeti</strong>
                <p style={{ margin: "5px 0 0", lineHeight: 1.55 }}>{pendingAiPlan.activityProcessNote}</p>
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "7px" }}>
                {pendingAiPlan.materialLabels.slice(0, 5).map((material) => (
                  <span key={material} style={{ padding: "6px 9px", borderRadius: "999px", background: "#e0f2fe", color: "#075985", fontSize: "0.78rem" }}>
                    {material}
                  </span>
                ))}
              </div>
            ...
            >
              <button type="button" onClick={() => setPendingAiPlan(null)} style={{ minHeight: "44px", padding: "0 16px", borderRadius: "10px", border: "1px solid #cbd5e1", background: "#ffffff", color: "#334155", fontWeight: 700 }}>
                Vazgeç
              </button>
              <button type="button" onClick={savePendingAiPlan} style={{ minHeight: "44px", padding: "0 16px", borderRadius: "10px", border: 0, background: "#047857", color: "#ffffff", fontWeight: 800 }}>
                Kontrol ettim, bugüne kaydet
              </button>
            </footer>
          </section>
        </div>
      ...
      <section className="today-context-hero-card" aria-label="Bugünün sınıf bilgisi">
        <div className="today-context-hero-header">
          ...
          <details className="today-context-tools">
            <summary>Veli iletişim araçları</summary>
            <div className="today-context-actions">
              <button
                type="button"
                className="today-export-btn today-export-btn--excel"
                onClick={() => void exportParentContactExcel(model.students, {
                  classroomName: classroom?.classroomName,
                  teacherName: classroom?.teacherName,
                  schoolName: "T.C. MİLLÎ EĞİTİM BAKANLIĞI",
                }).catch(() => setTodayAiNotice("❌ Excel çizelgesi oluşturulamadı. Bilgileri kontrol edip yeniden deneyin."))}
              >
                <FileTextIcon aria-hidden="true" />
                <span>Excel çizelgesi</span>
              </button>
              <button
                type="button"
