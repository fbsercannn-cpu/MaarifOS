...
const FOUNDER_CODE_PATTERN = /^\d{6}$/;
...
export interface PremiumFounderRedeemRequest {
  code: string;
  challengeId: string;
...
/**
 * Kurucu pilot PIN'i ticari MRF kodundan ayrı tutulur. Bu doğrulayıcıyı
 * genişletmek, ticari kod biçimini gevşetmek veya PIN'i kalıcılaştırmak yasaktır.
 */
export async function validatePremiumFounderRedeemRequest(
  value: unknown,
): Promise<Readonly<PremiumFounderRedeemRequest>> {
  assertLicenseApiRequestHasNoEducationalData(value);
  const request = object(value, "Kurucu erişimi kullanma isteği");
  exactKeys(
    request,
    [
      "appVersion",
    ...
    "Kurucu erişimi kullanma isteği",
  );
  const result: PremiumFounderRedeemRequest = {
    code: text(request.code, "Kurucu erişim kodu", FOUNDER_CODE_PATTERN),
    challengeId: text(request.challengeId, "Challenge kimliği", UUID_V4_PATTERN),
    proofSignature: text(request.proofSignature, "Cihaz ispat imzası", PROOF_SIGNATURE_PATTERN),
    ...(await verifiedDeviceFields(request)),
    appVersion: text(request.appVersion, "Uygulama sürümü", APP_VERSION_PATTERN),
