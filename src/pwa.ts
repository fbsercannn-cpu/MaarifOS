...
function ensureNativeRuntimeQuery(): void {
  ...
  const isInstalledApp = window.matchMedia?.("(display-mode: standalone)").matches === true;
  const explicitlyOpenedApp =
    url.searchParams.get("view") === "app" ||
    url.hash === "#app" ||
    url.pathname.startsWith("/gunum/");

  // The public root is the MaarifOS website. Only installed or explicitly
  // requested application sessions should be promoted to the native runtime.
  if (!isInstalledApp && !explicitlyOpenedApp) return;
