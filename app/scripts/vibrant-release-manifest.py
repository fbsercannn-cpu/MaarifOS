"""Bind the tested vibrant documents source to the final local 0.30 build and evidence."""
...
repo = app.parent
output = app / "output/class-roster-vibrant-2026-09-08/final"
...
def source_paths():
    return sorted([path for folder in ("src", "public", "scripts") for path in (app / folder).rglob("*") if path.is_file()] + [app / path for path in ("package.json", "package-lock.json", "mobile-runtime.lock.json", ".env.founder-production")])
...
assert receipt["status"] in ("VERIFIED_LOCAL", "VERIFIED_LIVE") and receipt["version"] == "0.30.0"
assert (app / "dist/client/index.html").is_file(), "Final built app is missing."
assert json.loads((app / "package.json").read_text(encoding="utf-8"))["version"] == "0.30.0"
...
manifest = {
    "version": "0.30.0", "status": receipt["status"], "published": receipt["status"] == "VERIFIED_LIVE",
